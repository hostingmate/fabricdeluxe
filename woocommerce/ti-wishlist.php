	...
	<form action="<?php echo esc_url( $form_url ); ?>" method="post" autocomplete="off">
		...
		<table class="tinvwl-table-manage-list">
			...
			<tr class="mb-3">
				...
				<th class="product-name ml-3"><span
			...
			foreach ( $products as $wl_product ) {
				...
				if ( $wl_product['quantity'] > 0 && apply_filters( 'tinvwl_wishlist_item_visible', true, $wl_product, $product ) ) {
					...
					?>
					<tr class="my-3 <?php echo esc_attr( apply_filters( 'tinvwl_wishlist_item_class', 'wishlist_item', $wl_product, $product ) ); ?>">
						<?php if ( isset( $wishlist_table['colm_checkbox'] ) && $wishlist_table['colm_checkbox'] ) { ?>
							<td class="product-cb">
								<?php
						...
						<td class="product-remove">
							<button type="submit" name="tinvwl-remove"
									...
									class="fal fa-times rounded-pill"></i>
							</button>
						</td>
